...
public class ChatRoomMemberAuthorityActivity extends GroupMemberAuthorityActivity {
    ...
    protected void onSubPrepareOptionsMenu(Menu menu) {
        ...
        if(!isOwner() && !isInAdminList(EaseIMHelper.getInstance().getCurrentUser())) {
    ...
    protected void initData() {
        chatRoom = EaseIMHelper.getInstance().getChatroomManager().getChatRoom(roomId);
                ...
                public void onSuccess(List<String> data) {
                    ...
                    if(data != null && !data.isEmpty()){
                        for(int i = 0; i < data.size(); i++){
                            EaseUser user = EaseIMHelper.getInstance().getUserInfo(data.get(i));
                            if(user != null){
                                users.add(user);
                            }else{
    ...
    public boolean isMember() {
        String currentUser = EaseIMHelper.getInstance().getCurrentUser();
